...
**Operating assumption on the rubric:** Treat the 100-pt split (Innovation 25 / Technical 25 / Impact 20 / Demo 20 / Presentation 10) as **provisional** — it has not been confirmed from the official SciBlitz rules. **Confirming it is move #1 below.** Until confirmed, every P0 below is chosen to score well under *both* that split and an Impact/feasibility-weighted split (IEEE-style). Do not over-invest in any single-rubric tactic until the real rubric is in hand.

---

## 1. If you do only 5 things before Jul 1

1. **Confirm the official rubric and re-read the rules page today** — verify the point split, the 5 required deliverables (live no-login URL, public repo, ≤8-pg PDF, 3–5 min video, 1-pg model card), and the Jun 23 registration deadline; one missing deliverable is a zero regardless of quality.
